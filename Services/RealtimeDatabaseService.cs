...
namespace IMP.Services
{
    public class RealtimeDatabaseService
    {
        ...
        // Metoda aktualizacji `elapsedTime` w Firebase
        public async Task UpdateElapsedTimeAsync(string userId, string sectionId, int elapsedTime)
        {
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    throw new Exception($"Failed to update elapsed time: {response.StatusCode}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error updating elapsed time: {ex.Message}");
                throw;
            }
        }
